# -*- coding: utf-8 -*-
"""perimeter_variant_code для gs_pd_federal_district_demand_params.
...
def upgrade():
    ...
    if not _column_exists(conn, "perimeter_variant_code"):
        ...
        op.create_index(
            "ix_gs_pd_federal_district_demand_params_pvc",
            TABLE,
    ...
    for ix in (
        ...
        "uq_gs_pd_federal_district_demand_params_hist",
        "uq_gs_pd_federal_district_demand_params_year",
    ):
...
def downgrade():
    ...
    conn.execute(
        text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_federal_district_demand_params_hist
            ON "{SCHEMA_PD}"."{TABLE}" (
                {FK_COL},
                COALESCE(database_version_id, 0)
            )
    ...
    conn.execute(
        text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_federal_district_demand_params_year
            ON "{SCHEMA_PD}"."{TABLE}" (
                {FK_COL},
                year_number,
                COALESCE(database_version_id, 0)
    ...
    if _column_exists(conn, "perimeter_variant_code"):
        if column_utils.index_exists(conn, SCHEMA_PD, "ix_gs_pd_federal_district_demand_params_pvc"):
            op.drop_index(
                "ix_gs_pd_federal_district_demand_params_pvc",
                table_name=TABLE,
                schema=SCHEMA_PD,
