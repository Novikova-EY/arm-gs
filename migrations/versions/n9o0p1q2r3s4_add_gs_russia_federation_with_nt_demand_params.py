...
def _ensure_indexes(connection, table_name: str) -> None:
    if not column_utils.index_exists(connection, SCHEMA_PD, "ix_gs_rf_nt_dp_year"):
        op.create_index(
            "ix_gs_rf_nt_dp_year",
            table_name,
            ["year_number"],
    ...
    if not column_utils.index_exists(connection, SCHEMA_PD, "ix_gs_rf_nt_dp_dbver"):
        op.create_index(
    ...
    op.execute(
        sa.text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_russia_federation_with_nt_demand_params_hist
    ...
    op.execute(
        sa.text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_russia_federation_with_nt_demand_params_year
            ON "{SCHEMA_PD}"."{table_name}" (
                year_number,
                COALESCE(database_version_id, 0)
            )
