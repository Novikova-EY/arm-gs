# -*- coding: utf-8 -*-
"""Таблица gs_ees_demand_params — параметры нагрузки «ЭЭС» (хаб модуля нагрузки).
...
def _ensure_indexes(connection, table_name: str) -> None:
    if not column_utils.index_exists(connection, SCHEMA_PD, "ix_gs_ees_dp_year"):
        op.create_index(
            "ix_gs_ees_dp_year",
            table_name,
            ["year_number"],
    ...
    if not column_utils.index_exists(connection, SCHEMA_PD, "ix_gs_ees_dp_dbver"):
        op.create_index(
            "ix_gs_ees_dp_dbver",
            table_name,
            ["database_version_id"],
    ...
    op.execute(
        sa.text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_ees_dp_hist
    ...
    op.execute(
        sa.text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_ees_dp_year
            ON "{SCHEMA_PD}"."{table_name}" (
                year_number,
                COALESCE(database_version_id, 0)
            )
