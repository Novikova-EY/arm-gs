# -*- coding: utf-8 -*-
"""Таблицы потребления ЭЭ по ВЭД (ФО и РФ) для долгосрочного прогноза.
...
TABLE_FD = "gs_ec_federal_district_economic_activity_consumption_params"
TABLE_RF = "gs_ec_russia_federation_economic_activity_consumption_params"
TABLE_FORMULA = "gs_ec_long_term_ved_consumption_formula_texts"
...
def upgrade():
    ...
    if not _table_exists(conn, SCHEMA_EC, TABLE_FORMULA):
        op.create_table(
            ...
            sa.UniqueConstraint("formula_key", name="uq_gs_ec_lt_ved_formula_texts_key"),
            schema=SCHEMA_EC,
        )
        op.create_index(
            "ix_gs_ec_lt_ved_formula_key",
            TABLE_FORMULA,
            ["formula_key"],
            unique=False,
            schema=SCHEMA_EC,
