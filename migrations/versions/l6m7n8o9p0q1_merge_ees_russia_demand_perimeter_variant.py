# -*- coding: utf-8 -*-
"""Одна таблица параметров ЕЭС России с perimeter_variant_code; перенос из with_nt; удаление второй таблицы.
...
SCHEMA_PD = "gs_pd"
SCHEMA_REF = "gs_sys"
TABLE = "gs_pd_ees_russia_demand_params"
TABLE_NT = "gs_pd_ees_russia_with_nt_demand_params"
...
def upgrade():
    ...
    if not _column_exists(conn, TABLE, "perimeter_variant_code"):
        ...
        op.create_index(
            "ix_gs_pd_ees_russia_demand_params_pvc",
            TABLE,
    ...
    for ix in (
        "uq_ees_russia_demand_params_hist",
        "uq_gs_ees_russia_demand_params_hist",
        "uq_ees_russia_demand_params_year",
        "uq_gs_ees_russia_demand_params_year",
    ):
    ...
    conn.execute(
        text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_gs_pd_ees_russia_demand_params_hist
            ON "{SCHEMA_PD}"."{TABLE}" (
    ...
    conn.execute(
        text(
            f"""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_gs_pd_ees_russia_demand_params_year
            ON "{SCHEMA_PD}"."{TABLE}" (
                year_number,
                COALESCE(database_version_id, 0),
                COALESCE(perimeter_variant_code, '')
