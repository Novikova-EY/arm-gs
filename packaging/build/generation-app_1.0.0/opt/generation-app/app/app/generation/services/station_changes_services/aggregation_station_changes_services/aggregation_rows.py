    ...
    for m in machines:
        ...
        for p in m.powers_by_year:
            ...
            rows.append(SimpleNamespace(
                ...
                station_type_id=station.id_station_type if station else None,
...
def get_full_aggregation_rows_old(start_year, end_year, station_ids, machine_ids=None):
    ...
    query = filter_by_explicit_db_version(query, MachinePower, current_db_version_id)
    query = filter_by_explicit_db_version(query, Machine, current_db_version_id)
    query = filter_by_explicit_db_version(query, Station, current_db_version_id)
    query = filter_by_explicit_db_version(query, MachineTesType, current_db_version_id)
    query = filter_by_explicit_db_version(query, TesType, current_db_version_id)
    query = filter_by_explicit_db_version(query, TesMachineType, current_db_version_id)
    query = filter_by_explicit_db_version(query, MachineFuel, current_db_version_id)
    query = filter_by_explicit_db_version(query, Fuel, current_db_version_id)
    query = filter_by_explicit_db_version(query, FuelType, current_db_version_id)
    query = filter_by_explicit_db_version(query, RegionalDistrict, current_db_version_id)
    query = filter_by_explicit_db_version(query, RegionalEnergySystem, current_db_version_id)
    query = filter_by_explicit_db_version(query, UnionEnergySystem, current_db_version_id)
    query = filter_by_explicit_db_version(query, EnergySystemType, current_db_version_id)
