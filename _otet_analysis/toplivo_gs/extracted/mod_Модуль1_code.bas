...
End Function
Public Function repbetw1(s As Variant, s1 As String, s2 As String, s3 As Variant) As Variant
i1 = InStr(1, s, s1, vbTextCompare)
i2 = InStr(1, Mid(s, i1 + Len(s1)), s2, vbTextCompare)
repbetw1 = Left(s, i1 + Len(s1) - 1) & s3 & Mid(s, i1 + Len(s1) + i2 - 1)
End Function
...
End If
End Function
Public Function setforsum()
Dim cv As Recordset
Dim work As QueryDef, dest As QueryDef, work1 As QueryDef, dopq As QueryDef
DoCmd.OpenForm "Выбор_варианта", , , , , acDialog
Set db = DBEngine.Workspaces(0).Databases(0)
Set cv = db.OpenRecordset("Текущий_вариант", DB_OPEN_DYNASET)
Set work = db.QueryDefs("Рабочий_для_суммы")
Set dopq = db.QueryDefs("Рабочий_для_суммы(Доп_угли)")
Set dest = db.QueryDefs("destination")
work.SQL = "select distinctrow " & "[" & cv!wname & "].* from " & "[" & cv!wname & "];"
dest.SQL = work.SQL
dopq.SQL = "select distinctrow " & "[" & cv!dopname & "].* from " & "[" & cv!dopname & "];"
End Function
...
Set db = DBEngine.Workspaces(0).OpenDatabase("f:\базы данных\ОТЭТ")
...
If (ob = 90) Or (ob = 91) Or (ob = 20) Or (ob = 21) Or (ob = 22) Or (ob = 23) Then typeob = ob
...
Sub Счет_Click()
Dim uform As Form, toplform As Form, dopform As Form
Dim db As Database, w As Form, U As Recordset, topl As Recordset
Dim dop As Recordset
...
ugli1 = ",nazar,ibor,berez,per,irbei,kansk,gusin,tugn,okino,azey,cher,jer,karab,mug,har,urt,tataur,tarbag,zab_kam,vork,intin,sver,chel,kizel,sosv,neru,zyryan,pyak,rai,erk,svo,bikin,razdol,hankai,bering,anad,ekib,maikub,kuznt,kuzngd,kuznss,kuznun,gazpp,"
...
Dim db As Database, R As Recordset, ws As Workspace, Q As QueryDef
nws = Workspaces.Count
Debug.Print nws
Set ws = CreateWorkspace("workspace" & Format(nws, "#"), "admin", "")
Workspaces.Append ws
Debug.Print Workspaces.Count
For i = 0 To Workspaces.Count - 1
Debug.Print "user="; Workspaces(i).Name
Next
Set db = Workspaces(1).OpenDatabase(DBEngine.Workspaces(0).Databases(0).Name)
Debug.Print db.Name
Set Q = db.QueryDefs("test")
Q.SQL = "select * from станции99;"
B = MsgBox("Вопрос", vbOKCancel)
End Function
...
'Dim sf As New ADODB.Recordset, df As New ADODB.Recordset, dfdef As TableDef, s As String
