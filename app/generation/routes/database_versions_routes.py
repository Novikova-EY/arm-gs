...
def add_database_version():
    ...
    form.parent_version_id.choices = [('', 'Новая пустая версия')] + [
    ...
    if request.method == "POST":
        ...
        try:
            # Преобразование parent_version_id: пустая строка или 'None' -> None, иначе int
            parent_id = form.parent_version_id.data
            if parent_id in (None, '', 'None', 'none'):
                parent_id = None
            else:
