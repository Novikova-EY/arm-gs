...
@prospective_places_bp.route("/aes/tep_main/")
@login_required
...
@prospective_places_bp.route("/aes/tep_reserve/")
@login_required
...
@prospective_places_bp.route("/aes/<int:id>/link_station/", methods=["POST"])
@login_required
...
@prospective_places_bp.route("/aes/<int:id>/go_station/", methods=["GET"])
@login_required
...
def _write_machines_sheet(ws, machines, include_place_name=False, include_note=True):
    ...
    h1_base = [
        ...
        "Удельные условно постоянные эксплуатационные затраты (без амортизационных отчислений) в ценах текущего года, тыс. руб. /кВт",
