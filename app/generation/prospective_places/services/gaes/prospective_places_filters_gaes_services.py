# -*- coding: utf-8 -*-
"""Сервис фильтрации перспективных площадок ГАЭС по ОЭС, РЭС, ФО, субъекту РФ."""
...
def _get_prospective_place_type_list():
    """Список типов перспективных площадок ГАЭС для фильтра."""
    from app.generation.prospective_places.models.gaes.prospective_place_type_gaes_model import ProspectivePlaceTypeGAES
    types = ProspectivePlaceTypeGAES.query.order_by(ProspectivePlaceTypeGAES.name).all()
...
def _get_distinct_site_names():
    ...
    rows = (
        db.session.query(StationProspectivePlaceGAES.site_name)
        .distinct()
        .order_by(StationProspectivePlaceGAES.site_name)
        .all()
    )
...
def _get_union_energy_systems_in_table():
    ...
    subq = (
        ...
        .join(StationProspectivePlaceGAES, StationProspectivePlaceGAES.id_regional_energy_system == RegionalEnergySystem.id)
    ...
    has_null = (
        db.session.query(StationProspectivePlaceGAES.id)
        .outerjoin(RegionalEnergySystem, StationProspectivePlaceGAES.id_regional_energy_system == RegionalEnergySystem.id)
        .filter(
            (StationProspectivePlaceGAES.id_regional_energy_system.is_(None)) |
...
def _get_regional_districts_in_table():
    ...
    subq = (
        db.session.query(StationProspectivePlaceGAES.id_regional_district)
        .filter(StationProspectivePlaceGAES.id_regional_district.isnot(None))
        .distinct()
    )
    rd_rows = (
    ...
    has_null = (
        db.session.query(StationProspectivePlaceGAES.id)
        .filter(StationProspectivePlaceGAES.id_regional_district.is_(None))
        .limit(1)
        .first()
    )
    if has_null:
...
def apply_prospective_places_gaes_station_order(query):
    """
    Сортировка площадок ГАЭС для списка и экспорта:
    ...
    return (
        query.outerjoin(
            ...
            StationProspectivePlaceGAES.id_regional_energy_system == RegionalEnergySystem.id,
        )
        .outerjoin(
            UnionEnergySystem,
        ...
        .order_by(
            ...
            nullslast(StationProspectivePlaceGAES.display_order.asc()),
            StationProspectivePlaceGAES.site_name.asc(),
        )
    )


def apply_prospective_places_gaes_filters(query, filters):
    """
    Применяет фильтры к запросу StationProspectivePlaceGAES (ГАЭС).
    ...
    if ues_filter:
        ...
        if ues_ids_real:
            conditions.append(
                StationProspectivePlaceGAES.regional_energy_system.has(
    ...
    if rd_ids:
        ...
        conditions = []
        if has_rd_empty:
            conditions.append(StationProspectivePlaceGAES.id_regional_district.is_(None))
        if rd_ids_real:
            conditions.append(StationProspectivePlaceGAES.id_regional_district.in_(rd_ids_real))
        if conditions:
            query = query.filter(or_(*conditions))
    ...
    if site_name_filter:
        ...
        if non_empty:
            conditions.append(StationProspectivePlaceGAES.site_name.in_(non_empty))
        if conditions:
            query = query.filter(or_(*conditions))
    ...
    if ppt_filter:
        query = query.filter(
            StationProspectivePlaceGAES.gaes_tep_source_indicators.any(
                ProspectivePlaceGaesTepSource.id_prospective_place_type_gaes.in_(ppt_filter)
            )
        )
...
def get_prospective_places_gaes_filter_context(filters):
...
def build_gaes_stations_grouped_by_place_type(stations):
    ...
    def group_sort_key(tid):
        ...
        if tid is None:
            ...
            tlabel = (sts[0].prospective_place_type_gaes.name or "").lower()
        else:
            tlabel = ""
        return (
