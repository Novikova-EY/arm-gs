...
def aggregate_changes_regional_energy_systems_by_station_types_with_events(rows):
    """
    Возвращает p_ust[res_id][station_type_id][event_code][year] = sum
    """
    from collections import defaultdict
    from decimal import Decimal
    ...
    for row in rows:
        ...
        if isinstance(events, (set, list, tuple)):
            for ev in events:
                p_ust[res][station_type][ev][year] += row.p_ust or Decimal(0)
        elif events:
            p_ust[res][station_type][events][year] += row.p_ust or Decimal(0)
