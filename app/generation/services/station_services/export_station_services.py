...
def generate_excel_export_with_all_totals(data, rows, start_year, end_year, rounding_digits, show_p_ogr=False, show_p_rasp=False, hide_aggregates=False, show_totals=True):
    ...
    def add_named_total_row(level_id, level_key, data, rows, start_year, end_year, round_val, show_p_ogr, show_p_rasp):
            ...
            if p_ust_dict and isinstance(p_ust_dict, dict) and len(p_ust_dict) > 0:
                ...
                if is_nested_structure:
                    ...
                    if current_version_id in p_ust_dict:
                        ...
                        # Если текущей версии нет, пробуем найти любую версию или используем первую доступную
                        available_versions = [k for k in p_ust_dict.keys() if isinstance(k, int) and (k < start_year or k > end_year)]
                        if available_versions:
                            fallback_version = available_versions[0]
                            p_ust_flat = p_ust_dict[fallback_version]
                            print(f"[WARNING] add_named_total_row для russia: version_id={current_version_id} не найден, использован ключ {fallback_version}")
                        else:
                            p_ust_flat = next(iter(p_ust_dict.values()))
                            print(f"[WARNING] add_named_total_row для russia: version_id={current_version_id} не найден, использован первый доступный ключ")
                else:
        ...
        for station_type_id, st_years in station_type_items_sorted:
            ...
            if station_type_name and "тэс" in station_type_name.lower():
                ...
                for res_id, res_group in ues_group.items():
                    ...
                    for rd_id, rd_group in res_group.items():
                        ...
                        for eu_id, eu_group in rd_group.items():
                            ...
                            # Показываем энергоузел только если имя не содержит "не указано" или "не указан"
                            show_energy_unit = bool(eu_name) and not any(x in str(eu_name).lower() for x in ["не указано", "не указан"])
                            if show_energy_unit:
                                row_eu = {
