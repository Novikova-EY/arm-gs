...
ESPP_ENERGY_GENERATION_PERIOD_YEAR = 0


class ESPPEnergyGeneration(db.Model, AuditMixin):
    __tablename__ = "gs_bem_espp_energy_generations"
    __table_args__ = (
        Index("ix_espp_energy_gen_id_regional_energy_system", "id_regional_energy_system"),
        Index("ix_espp_energy_gen_year_number", "year_number"),
        Index("ix_espp_energy_gen_month_number", "month_number"),
        Index(
            "ix_espp_energy_gen_res_year_month",
            "id_regional_energy_system",
            "year_number",
            "month_number",
        ),
        version_year_month_unique_index(
            "uq_espp_energy_gen_res_year_month_ver",
            "id_regional_energy_system",
        ),
        db.ForeignKeyConstraint(
            ...
            ondelete="RESTRICT",
            name="fk_espp_energy_gen_year_ver",
        ),
    ...
    year = db.relationship(
        "Year",
        back_populates="espp_energy_generations",
        lazy="noload",
        primaryjoin=(
    ...
    month_number = db.Column(
        db.Integer,
        nullable=False,
        default=ESPP_ENERGY_GENERATION_PERIOD_YEAR,
        server_default="0",
        index=True,
    )
    ...
    def is_annual(self) -> bool:
        return self.month_number == ESPP_ENERGY_GENERATION_PERIOD_YEAR

    def __repr__(self) -> str:
        period = "год" if self.is_annual else f"мес.{self.month_number}"
        return (
            f"<ESPPEnergyGeneration id={self.id} res_id={self.id_regional_energy_system} "
