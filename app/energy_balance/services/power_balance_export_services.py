# -*- coding: utf-8 -*-
"""CRUD значений строки «Экспорт мощности» на страницах баланса мощности."""
...
from app.energy_balance.models.power_balance_export_value_model import (
    EXPORT_ROW_KEY,
    PowerBalanceExportValue,
)
...
def _ensure_tables() -> None:
    ...
    try:
        ...
        PowerBalanceExportValue.__table__.create(bind, checkfirst=True)
        _tables_ready = True
    except Exception:
        pass
...
def _value_query():
    return filter_by_db_version(PowerBalanceExportValue.query, PowerBalanceExportValue)


def load_power_balance_export_inputs(
    years: list[int],
    sheets: list[dict[str, Any]] | None = None,
) -> dict[str, dict[str, dict[int, Decimal]]]:
    """{sheet_slug: {export: {year: value}}} для строк «Экспорт мощности»."""
    if not years:
        return {}
    _ensure_tables()
...
def update_power_balance_export_value(
    slug: str,
    year: int,
    raw_value: Any,
) -> dict[str, Any]:
    ...
    existing = (
        _value_query()
        .filter(
            PowerBalanceExportValue.sheet_slug == slug,
            PowerBalanceExportValue.year_number == year_int,
        )
        .one_or_none()
    )
    if parsed is None:
    ...
    if existing is None:
        existing = PowerBalanceExportValue(
            sheet_slug=slug,
