# -*- coding: utf-8 -*-
"""CRUD пользовательских строк «Получение/Передача мощности»."""
...
from app.energy_balance.models.power_balance_custom_flow_model import (
    CUSTOM_FLOW_DIRECTIONS,
    PowerBalanceCustomFlowRow,
    PowerBalanceCustomFlowValue,
)
...
def _ensure_tables() -> None:
    ...
    try:
        ...
        PowerBalanceCustomFlowRow.__table__.create(bind, checkfirst=True)
        PowerBalanceCustomFlowValue.__table__.create(bind, checkfirst=True)
...
def _ensure_parent_key_column(bind) -> None:
    try:
        ...
        schema = PowerBalanceCustomFlowRow.__table__.schema
        table = PowerBalanceCustomFlowRow.__tablename__
...
def _row_query():
    return filter_by_db_version(PowerBalanceCustomFlowRow.query, PowerBalanceCustomFlowRow)


def _serialize_row(row: PowerBalanceCustomFlowRow) -> dict[str, Any]:
    values: dict[int, Any] = {}
    for item in row.values or []:
        if item.year_number is None or item.value_mw is None:
            continue
        try:
            year = int(item.year_number)
        except (TypeError, ValueError):
            continue
        values[year] = item.value_mw
    return {
        "id": int(row.id),
...
def load_power_balance_custom_flows(sheets=None) -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        rows = (
            _row_query()
            .order_by(
                PowerBalanceCustomFlowRow.sheet_slug,
                PowerBalanceCustomFlowRow.direction,
                PowerBalanceCustomFlowRow.sort_order,
                PowerBalanceCustomFlowRow.id,
            )
            .all()
        )
...
def _ensure_vostok_layout_defaults(sheets) -> None:
    """Один раз переносит фиксированные строки ОЭС Востока в удаляемые пользовательские."""
    if not sheets:
        return
    from app.energy_balance.services.power_balance_page_services import (
    ...
    try:
        ...
        for slug in slugs:
            existing = (
                _row_query()
                .filter(PowerBalanceCustomFlowRow.sheet_slug == slug)
                .all()
            )
            seeded = False
...
def _get_row(slug: str, row_id: int) -> PowerBalanceCustomFlowRow | None:
    _ensure_tables()
    try:
        return (
            _row_query()
            .filter(
                PowerBalanceCustomFlowRow.id == int(row_id),
                PowerBalanceCustomFlowRow.sheet_slug == str(slug),
            )
            .one_or_none()
        )
    except (ProgrammingError, OperationalError):
...
def add_custom_flow_row(
    ...
    max_order = db.session.query(
        func.coalesce(func.max(PowerBalanceCustomFlowRow.sort_order), -1)
    ).filter(
        PowerBalanceCustomFlowRow.sheet_slug == slug,
        PowerBalanceCustomFlowRow.direction == direction,
        func.coalesce(PowerBalanceCustomFlowRow.parent_key, PowerBalanceCustomFlowRow.direction)
        == parent_key,
    )
    max_order = filter_by_db_version(max_order, PowerBalanceCustomFlowRow).scalar()
    next_order = int(max_order) + 1 if max_order is not None else 0
    row = PowerBalanceCustomFlowRow(
        sheet_slug=slug,
        direction=direction,
...
def _descendant_rows(slug: str, parent_key: str) -> list[PowerBalanceCustomFlowRow]:
    try:
        rows = (
            _row_query()
            .filter(PowerBalanceCustomFlowRow.sheet_slug == str(slug))
            .all()
        )
    except Exception:
        return []
    by_parent: dict[str, list[PowerBalanceCustomFlowRow]] = {}
    for item in rows:
        key = str(item.parent_key or item.direction or "")
        by_parent.setdefault(key, []).append(item)
    collected: list[PowerBalanceCustomFlowRow] = []
...
def update_custom_flow_value(
    ...
    if existing is None:
        existing = PowerBalanceCustomFlowValue(
            id_row=row.id,
