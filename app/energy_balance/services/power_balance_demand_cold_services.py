# -*- coding: utf-8 -*-
"""CRUD максимума потребления мощности для холодной пятидневки 0,92 (ТИТЭС)."""
...
from app.energy_balance.models.power_balance_demand_cold_value_model import (
    DEMAND_MAX_COLD_ROW_KEY,
    PowerBalanceDemandColdValue,
)
...
def _ensure_tables() -> None:
    ...
    try:
        ...
        PowerBalanceDemandColdValue.__table__.create(bind, checkfirst=True)
        _tables_ready = True
    except Exception:
        pass
...
def _value_query():
    return filter_by_db_version(
        PowerBalanceDemandColdValue.query, PowerBalanceDemandColdValue
    )


def load_power_balance_demand_cold_inputs(
    years: list[int],
    sheets: list[dict[str, Any]] | None = None,
) -> dict[str, dict[str, dict[int, Decimal]]]:
    """{sheet_slug: {demand_max_cold_092: {year: value}}}."""
    if not years:
        return {}
    _ensure_tables()
    ...
    for row in rows:
        ...
        if allowed is not None and slug not in allowed:
            continue
        if row.year_number is None or row.value_mw is None:
            continue
        try:
        ...
        if year not in wanted_years:
            continue
        inputs.setdefault(slug, {}).setdefault(DEMAND_MAX_COLD_ROW_KEY, {})[year] = Decimal(
            str(row.value_mw)
        )
    return inputs


def update_power_balance_demand_cold_value(
    slug: str,
    year: int,
    raw_value: Any,
) -> dict[str, Any]:
    ...
    existing = (
        _value_query()
        .filter(
            PowerBalanceDemandColdValue.sheet_slug == slug,
            PowerBalanceDemandColdValue.year_number == year_int,
        )
        .one_or_none()
    )
    if parsed is None:
    ...
    if existing is None:
        existing = PowerBalanceDemandColdValue(
            sheet_slug=slug,
