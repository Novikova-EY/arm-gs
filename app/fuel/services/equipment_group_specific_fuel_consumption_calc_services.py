# -*- coding: utf-8 -*-
"""
Расчёт полей _calc для EquipmentGroupSpecificFuelConsumption.
...
def calc_y_calc(param) -> Decimal:
    """
    Удельная выработка эл.эн. на тепловом потреблении (расчёт).
...
def calc_btp_calc(param, coeff_k) -> Decimal:
    """
    Удельный расход усл.топлива на отпуск эл.эн. в теплофик.режиме (расчёт).
...
def calc_sntp_calc(param) -> Decimal:
    """
    Эл.энергия на собственные нужды в теплофикационном режиме (расчёт).
...
def calc_bk_calc(param, btp_calc, sntp_calc) -> Decimal:
    """
    Удельный расход усл.топлива на отпуск эл.эн. в конд.режиме (расчёт).
