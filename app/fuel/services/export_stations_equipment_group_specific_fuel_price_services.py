# -*- coding: utf-8 -*-
"""Сервис экспорта цены для групп оборудования в Excel."""
...
def _format_cell_value(param, attr, is_numeric, rounding_digits):
    ...
    if param is None:
        return "—"
    if attr.endswith("_calc") and is_numeric:
        price_calc = getattr(param, "_price_calc", None) or {}
        val = price_calc.get(attr[:-5])
    else:
...
def export_stations_equipment_group_specific_fuel_price_to_excel(
    filters,
    start_year,
    end_year,
    *,
    ...
    """
    Экспортирует цену для групп оборудования в Excel.
    Учитывает фильтры: субъекты, ОЭС, региональные энергосистемы и т.п.
    """
    data = get_equipment_groups_with_specific_fuel_price_data(
        filters=filters,
        per_page=per_page,
        page=page,
    ...
    columns = ["Группа оборудования"] + [
        label for _, label, _ in SPECIFIC_FUEL_PRICE_COLUMNS
    ]
    ...
    for row_idx, (group_entity, param) in enumerate(rows, start=2):
        ...
        row_data = [group_name]
        for attr, _label, is_numeric in SPECIFIC_FUEL_PRICE_COLUMNS:
