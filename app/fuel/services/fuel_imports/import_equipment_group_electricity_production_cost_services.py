#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Сервис загрузки EquipmentGroupElectricityProductionCost из Excel.
Связь по столбцу NUMB1120 = EquipmentGroup.numb, code_zatr -> cost_code."""
...
INTEGER_FIELDS = frozenset(["year_number", "cost_code", "numb1120"])
STRING_FIELDS = frozenset(["name"])
NUMERIC_FIELDS = frozenset(["zatraty_sum", "zatr_e", "zatr_p", "zatr_q"])

COLUMN_ALIASES = {
    "name": ["name", "NAME", "наименование"],
    "year_number": ["year_number", "year", "Year", "YEAR", "год"],
    "cost_code": ["cost_code", "code_zatr", "codezatr"],
    "zatraty_sum": ["zatraty_sum", "zatr_sum", "ZATRATY_SUM"],
    "zatr_e": ["zatr_e", "Zatr_E", "zatr e"],
    "zatr_p": ["zatr_p", "Zatr_P", "zatr p"],
    "zatr_q": ["zatr_q", "Zatr_Q", "zatr q"],
...
def _apply_column_aliases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        canonical = alias_to_canonical.get(norm) or (
            norm if norm in IMPORT_FIELDS else None
        )
...
def import_equipment_group_electricity_production_cost_from_excel(
    ...
    """
    Загружает данные из Excel в EquipmentGroupElectricityProductionCost.
    Связь по NUMB1120 = EquipmentGroup.numb, code_zatr -> cost_code.
    """
    logger = _get_logger()
    ...
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_ELECTRICITY_PRODUCTION_COST] start user=%s filename=%s year=%s",
        user,
        filename,
        year,
    )
    ...
    for index, row in df.iterrows():
        ...
        cost_code = row_values.get("cost_code")
        if cost_code is None:
            skipped_no_cost_code += 1
            continue
        ...
        for equipment_group_id, eg_database_version_id in equipment_groups:
            ...
            if eg_database_version_id is not None:
                year_query = year_query.filter_by(
            ...
            for field, val in row_values.items():
                if hasattr(param, field) and field not in (
                    "year_number",
                    "cost_code",
                ):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception(
            "[IMPORT_EQUIPMENT_GROUP_ELECTRICITY_PRODUCTION_COST] db error: %s", exc
        )
        raise ValueError(
    ...
    message = (
        ...
        skipped_no_cost_code=skipped_no_cost_code,
        skipped_no_year=skipped_no_year,
    )
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_ELECTRICITY_PRODUCTION_COST] done created=%s updated=%s "
        "skipped=%s skipped_no_cost_code=%s skipped_no_year=%s elapsed=%.2fs",
        created,
        updated,
        skipped_no_match,
        skipped_no_cost_code,
        skipped_no_year,
        elapsed,
    )
    log_to_db(user, "Загрузка EquipmentGroupElectricityProductionCost", message)
    return {
        "message": message,
        "created": created,
        "updated": updated,
