...
Сервис загрузки расчетных значений удельных показателей (y, btp, sntp, bk, snk) из Excel
...
def import_equipment_group_specific_fuel_consumption_from_excel(
    ...
    По NUMB ищем EquipmentGroup.numb, создаем/обновляем consumption для года year.
...
def import_equipment_group_specific_fuel_consumption_calculated_from_excel(
    ...
    """
    Загружает расчетные значения удельных показателей (name, year_number, y, btp, sntp, bk, snk)
    ...
    log_to_db(user, "Загрузка расчетных значений удельных показателей", message)
    return {
        "message": message,
        "created": created,
        "updated": updated,
