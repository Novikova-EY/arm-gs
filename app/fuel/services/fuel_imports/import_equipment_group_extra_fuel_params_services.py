#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Сервис загрузки EquipmentGroupExtraFuelParam из Excel.
...
EQUIPMENT_GROUP_EXTRA_FUEL_PARAM_FIELDS = [
    "name",
...
INTEGER_FIELDS = frozenset(["numb1120", "numb1"])
...
def _apply_column_aliases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for field in EQUIPMENT_GROUP_EXTRA_FUEL_PARAM_FIELDS:
    ...
    for col in df.columns:
        ...
        canonical = alias_to_canonical.get(norm) or (
            norm if norm in EQUIPMENT_GROUP_EXTRA_FUEL_PARAM_FIELDS else None
        )
...
def import_equipment_group_extra_fuel_params_from_excel(file, user: str, year: int) -> dict:
    """
    Загружает данные из Excel в EquipmentGroupExtraFuelParam.
    Связь по столбцу numb1120 = EquipmentGroup.numb.
    """
    logger = _get_logger()
    ...
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_EXTRA_FUEL_PARAMS] start user=%s filename=%s year=%s",
        user, filename, year,
    )
    ...
    for index, row in df.iterrows():
        ...
        for equipment_group_id, eg_database_version_id in equipment_groups:
            ...
            param_query = EquipmentGroupExtraFuelParam.query.filter_by(
                equipment_group_id=equipment_group_id,
                year_number=year,
            )
            param = param_query.first()
            if param is None:
                param = EquipmentGroupExtraFuelParam(
                    equipment_group_id=equipment_group_id,
                    year_number=year,
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("[IMPORT_EQUIPMENT_GROUP_EXTRA_FUEL_PARAMS] db error: %s", exc)
        raise ValueError("Ошибка сохранения данных: проверьте корректность файла.") from exc

    elapsed = time.perf_counter() - t0
    message = (
        "Загрузка данных в EquipmentGroupExtraFuelParam завершена. "
    ...
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_EXTRA_FUEL_PARAMS] done created=%s updated=%s skipped=%s skipped_no_year=%s elapsed=%.2fs",
        created, updated, skipped_no_match, skipped_no_year, elapsed,
    )
    log_to_db(user, "Загрузка EquipmentGroupExtraFuelParam", message)
    return {
        "message": message,
        "created": created,
        "updated": updated,
