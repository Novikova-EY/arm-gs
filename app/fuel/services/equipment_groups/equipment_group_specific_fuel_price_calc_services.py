# -*- coding: utf-8 -*-
"""
Расчет полей xxx_c модели EquipmentGroupSpecificFuelPrice по формулам.
...
# Поля, для которых quantity берется из EquipmentGroupFuelParam (STAN)
...
# Поля, для которых quantity берется из EquipmentGroupExtraFuelParam (DUG)
FROM_EXTRA_FUEL = [
    "gazpp",
    "gaz_prir",
    "disel",
...
def calc_specific_fuel_price_fields(fuel_param, extra_fuel_param, cost):
    ...
    Returns:
        dict: {attr_c: Decimal, ...} для всех расчетных полей
    """
    result = {}
    if cost is None:
        return result
