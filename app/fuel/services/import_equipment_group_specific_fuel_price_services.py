#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Сервис загрузки EquipmentGroupSpecificFuelPrice из Excel.
...
SPECIFIC_FUEL_PRICE_FIELDS = [
    "name", "year_number", "obl",
    "gaz_c", "gazpp_c", "gaz_prir_c", "mazut_c", "disel_c", "maztop_c",
    "gtt_c", "nft_proch_c", "torf_c", "isk_gaz_c", "domen_g_c", "koks_g_c",
    "prochgaz_c", "proch_c", "tvproch_c", "szh_gaz_c", "inoe_c",
    "don_c", "podm_c", "pech_c", "vork_c", "intin_c",
    "kuzn_c", "kuzngd_c", "kuznt_c", "kuznss_c", "kuznun_c",
    "ural_c", "sver_c", "chel_c", "kizel_c", "bashk_c", "kazah_c",
    "ekib_c", "maikub_c", "karag_c", "karajyra_c", "teniz_c",
    "kan_c", "nazar_c", "ibor_c", "berez_c", "per_c", "irbei_c", "kansk_c",
    "irkut_c", "azey_c", "mug_c", "cher_c", "tung_c", "jer_c", "karab_c",
    "hak_c", "tuv_c", "bur_c", "gusin_c", "tugn_c", "okino_c",
    "chit_c", "har_c", "urt_c", "tataur_c", "tarbag_c", "zab_kam_c",
    "amur_c", "rai_c", "erk_c", "ogodj_c", "svo_c", "urg_c", "ushum_c",
    "prim_c", "bikin_c", "razdol_c", "hankai_c", "yakut_c",
    "neru_c", "zyryan_c", "pyak_c", "mag_c", "chukot_c", "anad_c",
    "bering_c", "kamch_c", "sah_c",
    "numb1120", "sost", "group",
]

INTEGER_FIELDS = frozenset(["year_number", "numb1120"])

STRING_FIELDS = frozenset(["name", "obl", "sost", "group"])
...
def _apply_column_aliases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for field in SPECIFIC_FUEL_PRICE_FIELDS:
    ...
    for col in df.columns:
        ...
        canonical = alias_to_canonical.get(norm) or (
            norm if norm in SPECIFIC_FUEL_PRICE_FIELDS else None
        )
...
def import_equipment_group_specific_fuel_price_from_excel(
    file, user: str, year: int
) -> dict:
    """
    Загружает данные из Excel в EquipmentGroupSpecificFuelPrice.
    Связь по столбцу numb1120 = EquipmentGroup.numb.
    Год из формы используется, если year_number не указан в строке Excel.
    """
    logger = _get_logger()
    ...
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_SPECIFIC_FUEL_PRICE] start user=%s filename=%s year=%s",
        user, filename, year,
    )
    ...
    for index, row in df.iterrows():
        ...
        for field in SPECIFIC_FUEL_PRICE_FIELDS:
            ...
            if field in INTEGER_FIELDS:
                ...
                if val and len(val) > 512 and field == "name":
                    val = val[:512]
                if val and len(val) > 80 and field in ("obl", "sost", "group"):
                    val = val[:80]
            else:
                val = str(raw).strip() if raw else None
            if val is not None:
        ...
        row_year = row_values.get("year_number") or year
        ...
        for equipment_group_id, eg_database_version_id in equipment_groups:
            ...
            param_query = EquipmentGroupSpecificFuelPrice.query.filter_by(
                equipment_group_id=equipment_group_id,
                year_number=row_year,
            )
            param = param_query.first()
            if param is None:
                param = EquipmentGroupSpecificFuelPrice(
                    equipment_group_id=equipment_group_id,
                    year_number=row_year,
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("[IMPORT_EQUIPMENT_GROUP_SPECIFIC_FUEL_PRICE] db error: %s", exc)
        raise ValueError("Ошибка сохранения данных: проверьте корректность файла.") from exc

    elapsed = time.perf_counter() - t0
    message = (
        "Загрузка данных в EquipmentGroupSpecificFuelPrice завершена. "
    ...
    logger.info(
        "[IMPORT_EQUIPMENT_GROUP_SPECIFIC_FUEL_PRICE] done created=%s updated=%s skipped=%s skipped_no_year=%s elapsed=%.2fs",
        created, updated, skipped_no_match, skipped_no_year, elapsed,
    )
    log_to_db(user, "Загрузка EquipmentGroupSpecificFuelPrice", message)
    return {
        "message": message,
        "created": created,
        "updated": updated,
