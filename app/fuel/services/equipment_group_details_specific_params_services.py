...
# Формулы расчёта для отображения во всплывающей подсказке (attr -> формула)
...
# Consumption: k (по году) и расчётные параметры (_calc)
CONSUMPTION_SPEC = [
    (attr, label) for attr, label, is_numeric in SPECIFIC_FUEL_CONSUMPTION_COLUMNS
    if attr == "k" or (is_numeric and attr.endswith("_calc"))
]
...
# Price: только расчётные параметры (_calc)
PRICE_SPEC = [
...
def build_table4_consumption_grid(
    ...
    без пересчёта при загрузке страницы.
