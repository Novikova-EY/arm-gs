#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Сервисы для страницы «Удельные показатели групп оборудования»."""
...
SPECIFIC_FUEL_CONSUMPTION_COLUMNS = [
    ...
    ("snk", "SNK", True),
    ("y_calc", "Y (расчёт)", True),
    ("btp_calc", "BTP (расчёт)", True),
    ("sntp_calc", "SNTP (расчёт)", True),
    ("bk_calc", "BK (расчёт)", True),
    ("snk_calc", "SNK (расчёт)", True),
]
...
def build_equipment_group_specific_fuel_consumption_hierarchy(rows):
    ...
    NUMERIC_ATTRS = ["y", "btp", "sntp", "bk", "snk", "y_calc", "btp_calc", "sntp_calc", "bk_calc", "snk_calc"]
