# -*- coding: utf-8 -*-
"""Экспорт калорийных эквивалентов топлива групп оборудования в Excel."""
...
def export_stations_equipment_group_natural_fuel_to_excel(
    filters,
    start_year,
    end_year,
    *,
    ...
    """Экспорт калорийных эквивалентов топлива с теми же фильтрами, что на странице."""
    data = get_equipment_groups_with_natural_fuel_data(
        filters=filters,
        per_page=per_page,
        page=page,
    ...
    hierarchy = build_equipment_group_natural_fuel_hierarchy(rows)
    if not hierarchy:
        return None

    wb = Workbook()
    ws = wb.active
    ws.title = "Калорийные эквиваленты"

    columns = [EQUIPMENT_GROUP_ID_HEADER, "Группа оборудования"] + [
        label for _, label, _ in NATURAL_FUEL_COLUMNS
    ]
                ...
                for station_block in res_block.get("station_blocks") or []:
                    for group_block in station_block.get("group_blocks") or []:
                        ...
                        for _eg, param in group_block.get("rows") or []:
                            ...
                            for attr, _label, is_numeric in NATURAL_FUEL_COLUMNS:
                                row_data.append(
