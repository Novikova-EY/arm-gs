# -*- coding: utf-8 -*-
"""Сервис экспорта удельных показателей групп оборудования в Excel."""
...
def export_stations_equipment_group_specific_fuel_consumption_to_excel(
    ...
    *,
    ...
    """
    Экспортирует удельные показатели групп оборудования в Excel.
    ...
    """
    data = get_equipment_groups_with_specific_fuel_consumption_data(
        filters=filters,
        per_page=per_page,
        page=page,
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Удельные показатели"

    columns = [EQUIPMENT_GROUP_ID_HEADER, "Группа оборудования"] + [
        label for _, label, _ in SPECIFIC_FUEL_CONSUMPTION_COLUMNS
    ]
