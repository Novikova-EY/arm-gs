# -*- coding: utf-8 -*-
"""Сервис экспорта затрат ТЭС на производство ЭЭ и ТЭ в Excel."""
...
def _format_cell_value(param, attr, is_numeric, rounding_digits, cost_code_name_map):
    if param is None:
        return "—"
    if attr == "cost_name":
        code = getattr(param, "cost_code", None)
        if code is None:
            return "—"
        return cost_code_name_map.get(code, "—")
...
def _format_summary_value(summary, attr, is_numeric, rounding_digits):
    if summary is None or attr not in summary or attr == "cost_name":
        return "—"
...
def export_stations_equipment_group_electricity_production_cost_to_excel(
    filters,
    start_year,
    end_year,
    *,
    ...
    data = get_equipment_groups_with_electricity_production_cost_data(
        filters=filters,
        per_page=per_page,
        page=page,
    ...
    hierarchy = build_equipment_group_electricity_production_cost_hierarchy(rows)
    if not hierarchy:
        return None
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Затраты ТЭС"

    columns = [EQUIPMENT_GROUP_ID_HEADER, "Группа оборудования"] + [
        label for _, label, _ in ELECTRICITY_PRODUCTION_COST_COLUMNS
    ]
                ...
                for station_block in res_block.get("station_blocks") or []:
                    for group_block in station_block.get("group_blocks") or []:
                        ...
                        for _eg, param in group_block.get("rows") or []:
                            ...
                            for attr, _label, is_numeric in ELECTRICITY_PRODUCTION_COST_COLUMNS:
                                row_data.append(
                                    _format_cell_value(
                                        param,
                                        attr,
