...
class FuelRestriction(db.Model):
    ...
    # Минимальная выработка ЭЭ, тыс.кВтч (emin)
    emin = db.Column(db.Numeric(36, 16), nullable=True)
    # Максимальная выработка ЭЭ, тыс.кВтч (emax)
    emax = db.Column(db.Numeric(36, 16), nullable=True)
    # Текущая выработка ЭЭ, тыс.кВтч (ecur)
    ecur = db.Column(db.Numeric(36, 16), nullable=True)
    # ЧЧИУМ, ч (Access H)
    h = db.Column("h", db.Numeric(36, 16), nullable=True)
    # Текущая выработка ЭЭ с учетом ограничения, тыс.кВтч (ecurdis)
    ecurdis = db.Column(db.Numeric(36, 16), nullable=True)
    # ЧЧИУМ с учетом ограничения, ч (hdis)
    hdis = db.Column(db.Numeric(36, 16), nullable=True)
    # Теплофикационная выработка ЭЭ, тыс.кВтч (etp); расчёт на «Распред»:
    # Σ ewtp по obl в фильтре параметра за расчётный год (после ewtp=qotr·y/1000).
...
FUEL_RESTRICTION_LIST_COLUMN_HEADINGS: list[tuple[str, str, str]] = [
    ...
    ("emin", "Минимальная выработка ЭЭ, тыс.кВтч", "emin"),
    ("emax", "Максимальная выработка ЭЭ, тыс.кВтч", "emax"),
    ("ecur", "Текущая выработка ЭЭ, тыс.кВтч", "ecur"),
    ("h", "ЧЧИУМ, ч", "H"),
    ("ecurdis", "Текущая выработка ЭЭ с учетом ограничения, тыс.кВтч", "ecurdis"),
    ("hdis", "ЧЧИУМ с учетом ограничения, ч", "Hdis"),
    ("etp", "Теплофикационная выработка ЭЭ, тыс.кВтч", "etp"),
    ("kobl", "Коэффициент по субъекту", "kobl"),
    ("kcur", "Текущий коэффициент", "kcur"),
]

# Подсказки-формулы для шапок расчётных колонок (иконка i).
# Дефолты; эффективные тексты — get_restriction_column_formulas().
FUEL_RESTRICTION_COLUMN_FORMULAS: dict[str, str] = {
    "etp": (
