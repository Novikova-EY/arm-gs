...
PAGE_LABELS: dict[str, str] = {
    PAGE_SUMMARY_TABLE: "Сводная таблица (/summary_table/)",
...
class EcSummaryFormulaDef:
    ...
    def page_label_lines(self) -> tuple[str, ...]:
        """Подписи страниц по одной на строку (для столбца и фильтра «Страница»).

        Без завершающих «;»: иначе в фильтре появляются дубли
        («…/fo/» и «…/fo/;»), когда одна и та же страница в разных
        формулах бывает и последней, и не последней в списке.
        """
        if self.pages == ALL_SUMMARY_PAGES:
            return ("Все перечисленные сводки",)
        return tuple(PAGE_LABELS[p] for p in sorted(self.pages))
...
EC_SUMMARY_FORMULA_REGISTRY: tuple[EcSummaryFormulaDef, ...] = (
    ...
    _def(
        ...
        cell_name="Потребление ЭЭ энергозоны, млн кВтч",
        default_text=(
    ...
    _def(
        ...
        pages=frozenset({PAGE_EZ, PAGE_SUMMARY_TABLE}),
        aggregation_level="Энергозона",
        cell_name="Потребление ЭЭ энергозоны (СиПР), млн кВтч",
        default_text=(
            "Потребление ЭЭ энергозоны (СиПР), млн кВтч = сумма потребления "
    ...
    _def(
        ...
        cell_name="Потребление ЭЭ по ФО, млн кВтч",
        default_text=(
            "Потребление ЭЭ по ФО, млн кВтч = сумма потребления ЭЭ всех РЭС, "
    ...
    _def(
        ...
        pages=frozenset({PAGE_FO, PAGE_SUMMARY_TABLE}),
        aggregation_level="Федеральный округ",
        cell_name="Потребление ЭЭ по ФО (СиПР), млн кВтч",
        default_text=(
            "Потребление ЭЭ по ФО (СиПР), млн кВтч = сумма потребления ЭЭ (СиПР) "
    ...
    _def(
        ...
        default_text=(
            "Потребление электрической энергии без заряда ГАЭС, млн кВтч = "
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="ОЭС Юга без НТ без заряда ГАЭС",
        default_text=(
            "ОЭС Юга без НТ без заряда ГАЭС = "
            "ОЭС Юга без НТ с зарядом ГАЭС − заряд ГАЭС"
        ),
    ),
    _def(
        "gaes_adj_ez",
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="ЕЭС России с НТ без заряда ГАЭС",
        default_text=(
            "ЕЭС России с НТ без заряда ГАЭС = "
            "ЕЭС России с НТ с зарядом ГАЭС − Заряд ГАЭС"
        ),
    ),
    _def(
        "gaes_adj_ees_russia_without_nt",
        aggregation_level="Без заряда ГАЭС",
        cell_name="ЕЭС России без НТ без заряда ГАЭС",
        default_text=(
            "ЕЭС России без НТ без заряда ГАЭС = "
            "ЕЭС России без НТ с зарядом ГАЭС − Заряд ГАЭС"
        ),
    ),
    _def(
        "gaes_adj_sa",
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="Первая СЗ с НТ (колонка без заряда ГАЭС)",
        default_text=(
            "Первая синхронная зона с НТ без заряда ГАЭС, млн кВтч = "
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="Первая СЗ с НТ без ЭС Калининграда (колонка без заряда ГАЭС)",
        default_text=(
            "Первая синхронная зона с НТ без заряда ГАЭС, млн кВтч = "
            "Первая синхронная зона с НТ с зарядом ГАЭС − заряд ГАЭС"
        ),
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="Первая СЗ без НТ с ЭС Калининграда (колонка без заряда ГАЭС)",
        default_text=(
    ...
    _def(
        ...
        aggregation_level="Без заряда ГАЭС",
        cell_name="Первая СЗ без НТ без ЭС Калининграда (колонка без заряда ГАЭС)",
        default_text=(
    ...
    _def(
        ...
        aggregation_level="Синхронная зона",
        cell_name="Первая СЗ без НТ с зарядом ГАЭС (без ЭС Калининграда)",
        default_text=(
    ...
    _def(
        ...
        default_text=(
            "Потребление ЕЭС России с НТ, млн кВтч = сумма потребления ЭЭ всех ОЭС + "
    ...
    _def(
        ...
        pages=frozenset({PAGE_OES, PAGE_SUMMARY_TABLE, PAGE_OES_GAES_CHARGE}),
        aggregation_level="ЕЭС России",
        cell_name="Потребление ЕЭС России, млн кВтч",
        default_text="Потребление ЕЭС России, млн кВтч = сумма потребления ЭЭ всех ОЭС",
    ),
    _def(
        "oes_ees_russia_with_nt_mln",
        pages=frozenset({PAGE_OES, PAGE_SUMMARY_TABLE, PAGE_OES_GAES_CHARGE}),
        aggregation_level="ЕЭС России",
        cell_name="Потребление ЕЭС России с НТ, млн кВтч",
        default_text=(
            "Потребление ЕЭС России с НТ, млн кВтч = сумма потребления ЭЭ всех ОЭС + "
            "потребление ЭЭ Новыми территориями"
        ),
    ...
    _def(
        ...
        cell_name="Потребление ТИТЭС, млн кВтч",
        default_text=(
            ...
            "только до 2018 года включительно"
        ),
    ),
    _def(
        "nt_under_south",
    ...
    _def(
        ...
        default_text=(
            "Строка «ЕЭС России без НТ с зарядом ГАЭС» заполняется вручную "
            "(без автоформулы). Сверка с 1-й СЗ / 2-й СЗ / ТИТЭС — в строке "
            "«Проверка ЕЭС России без НТ с зарядом ГАЭС»."
        ),
    ),
    _def(
        "ezs_russia_with_nt_with_gaes",
        pages=frozenset({PAGE_OES, PAGE_SUMMARY_TABLE, PAGE_OES_GAES_CHARGE}),
        aggregation_level="ЭЭС России",
        cell_name="ЭЭС России с НТ с зарядом ГАЭС",
        default_text=(
    ...
    _def(
        "ezs_russia_without_nt_with_gaes",
        pages=frozenset({PAGE_OES, PAGE_SUMMARY_TABLE, PAGE_OES_GAES_CHARGE}),
        aggregation_level="ЭЭС России",
        cell_name="ЭЭС России без НТ с зарядом ГАЭС",
        default_text=(
    ...
    _def(
        ...
        default_text=(
            "Строка «ЕЭС России с НТ с зарядом ГАЭС» заполняется вручную "
            "(без автоформулы). Сверка с составляющими — в строке "
    ...
    _def(
        ...
        default_text="Потребление Вторая синхронная зона, млн кВтч = ОЭС Востока",
    ),
    _def(
    ...
    _def(
        ...
        pages=frozenset({PAGE_OES, PAGE_SUMMARY_TABLE, PAGE_OES_GAES_CHARGE}),
        aggregation_level="Проверка",
        cell_name="Проверка первой синхронной зоны без НТ с зарядом ГАЭС",
        default_text=(
    ...
    _def(
        ...
        default_text=(
            "Электроёмкость (расчётная), кВтч/тыс. руб., по каждому году (не позже года "
    ...
    _def(
        "ei_rf_total_consumption",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_TOTAL_CONSUMPTION],
    ...
    _def(
        ...
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_GROWTH_RATE],
        default_text=EI_RF_GROWTH_RATE_FORMULA_TOOLTIP,
    ),
    _def(
        "ei_rf_summary_ved_consumption",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_VED_CONSUMPTION],
        default_text=EI_RF_SUMMARY_VED_CONSUMPTION_FORMULA_TOOLTIP,
    ),
    _def(
        "ei_rf_ved_consumption",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ (ВЭД)",
        cell_name=REF_ROW_LABEL_BY_KIND[REF_ROW_CONSUMPTION],
        default_text=EI_RF_VED_CONSUMPTION_FORMULA_TOOLTIP,
    ),
    _def(
        "ei_rf_pop_household_consumption",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ (Население)",
    ...
    _def(
        ...
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_GAES],
        default_text=EI_RF_GAES_FORMULA_TOOLTIP,
    ),
    _def(
        "ei_rf_consumption_without_gaes",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_CONSUMPTION_WITHOUT_GAES],
        default_text=EI_RF_CONSUMPTION_WITHOUT_GAES_FORMULA_TOOLTIP,
    ),
    _def(
        "ei_rf_network_losses",
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_NETWORK_LOSSES],
    ...
    _def(
        ...
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_POWER_STATION],
    ...
    _def(
        ...
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_GDP],
    ...
    _def(
        ...
        pages=frozenset({PAGE_ELECTRICAL_INTENSITY}),
        aggregation_level="РФ",
        cell_name=REF_ROW_RF_LABEL_BY_KIND[REF_ROW_RF_GDP_INTENSITY],
