...
from app.energy_consumption.long_term_consumption.models.federal_district_electrical_intensity_coefficient_model import (
    FederalDistrictElectricalIntensityCoefficient,
)
from app.energy_consumption.long_term_consumption.models.federal_district_electrical_intensity_year_parameter_model import (
    FederalDistrictElectricalIntensityYearParameter,
)
from app.energy_consumption.long_term_consumption.models.federal_district_population_consumption_coefficient_model import (
    FederalDistrictPopulationConsumptionCoefficient,
)
from app.energy_consumption.long_term_consumption.models.federal_district_population_consumption_year_parameter_model import (
    FederalDistrictPopulationConsumptionYearParameter,
)
from app.energy_consumption.long_term_consumption.models.russia_federation_electrical_intensity_coefficient_model import (
    RussiaFederationElectricalIntensityCoefficient,
)
from app.energy_consumption.long_term_consumption.models.russia_federation_electrical_intensity_year_parameter_model import (
...
from app.energy_consumption.long_term_consumption.services.formula_text.electrical_intensity_formula_text_services import (
    ei_formula_text,
)
from app.energy_consumption.long_term_consumption.services.electrical_intensity_chart_services import (
...
def _ei_years_through_current(
    ...
    """Годы, для которых электроёмкость и расчётная строка считаются по формулам."""
...
def _force_compute_graph_points_for_rows(
    ...
    *,
    ...
    for year in computed_years:
        ...
        if raw_gp is None:
            continue
        qv = _quantize_ei_value(raw_gp, EI_GRAPH_POINT_ROUNDING_DIGITS)
        _set_row_cell_value(
            graph_point_row,
            year,
            qv,
...
def _enrich_ei_computed_rows(
    ...
    *,
    ...
    for year in computed_years:
        ...
        if auto_fill_graph_points and graph_point_row is not None:
            if graph_point_row.get("cells", {}).get(year) is None:
                raw_gp = _compute_ei_graph_point(
                    intensity_curr,
                    intensity_prev,
                    accum,
                ...
                if raw_gp is not None:
                    _set_row_cell_value(
                        ...
                        year,
                        _quantize_ei_value(
                            raw_gp, EI_GRAPH_POINT_ROUNDING_DIGITS
                        ),
                        display_years=display_years,
                        rounding_digits=rounding_digits,
                    )
        ...
        if intensity_row is not None:
            ...
            if raw is not None:
                _set_row_cell_value(
                    ...
                    display_years=display_years,
                    rounding_digits=rounding_digits,
                    tooltip_value=_quantize_ei_value(
                        raw,
                        EI_INTENSITY_TOOLTIP_ROUNDING_DIGITS,
        ...
        if calculated_row is not None:
            ...
            if raw is not None:
                _set_row_cell_value(
                    ...
                    year,
                    _quantize_ei_value(
                        raw, rounding_digits, row_kind=ROW_KIND_CALCULATED
                    ),
                    display_years=display_years,
                    rounding_digits=rounding_digits,
                )
    ...
    if calculated_row is not None:
        calculated_row["computed_years"] = computed_years

    if delta_row is not None:
        delta_computed_years: set[int] = set()
        for year in computed_years:
            intensity_val = (
                intensity_row.get("cells", {}).get(year) if intensity_row else None
            )
            calculated_val = (
                calculated_row.get("cells", {}).get(year) if calculated_row else None
            )
            if intensity_val is None or calculated_val is None:
                continue
            delta_val = intensity_val - calculated_val
            _set_row_cell_value(
                delta_row,
                year,
                _quantize_ei_value(
                    delta_val, rounding_digits, row_kind=ROW_KIND_DELTA
                ),
                display_years=display_years,
                rounding_digits=rounding_digits,
            )
...
def _load_population_year_values_map(
    *,
    ...
    q = FederalDistrictPopulationConsumptionYearParameter.query.filter(
...
def _load_population_coef(
    *,
    ...
    q = FederalDistrictPopulationConsumptionCoefficient.query.filter(
        FederalDistrictPopulationConsumptionCoefficient.id_federal_district == fd_id
    )
    if version_id is not None:
        q = q.filter(
...
def _enrich_population_computed_rows(
    ...
    *,
    ...
    for year in computed_years:
            ...
            if graph_point_row.get("cells", {}).get(year) is None:
                ...
                if raw_gp is not None:
                    _set_row_cell_value(
                        graph_point_row,
                        year,
                        _quantize_ei_value(raw_gp, EI_GRAPH_POINT_ROUNDING_DIGITS),
                        display_years=display_years,
                        rounding_digits=rounding_digits,
                    )
        ...
        if intensity_row is not None and per_capita_curr is not None:
            _set_row_cell_value(
                ...
                year,
                _quantize_ei_value(
                    per_capita_curr, rounding_digits, row_kind=ROW_KIND_INTENSITY
                ),
                display_years=display_years,
                rounding_digits=rounding_digits,
                tooltip_value=_quantize_ei_value(
    ...
    for year in computed_years:
        accum = accum_income_by_year.get(year)
        if calculated_row is not None:
            raw = _compute_ei_calculated(coef_a, effective_coef_x, accum)
            if raw is not None:
                _set_row_cell_value(
                    calculated_row,
                    year,
    ...
    if delta_row is not None:
        ...
        for year in computed_years:
            ...
            if intensity_val is None or calculated_val is None:
                continue
            delta_val = calculated_val - intensity_val
            _set_row_cell_value(
                delta_row,
                year,
                _quantize_ei_value(delta_val, rounding_digits, row_kind=ROW_KIND_DELTA),
                display_years=display_years,
                rounding_digits=rounding_digits,
            )
            delta_computed_years.add(year)
...
def _refdata_ved_types_for_version(version_id: int | None) -> list[EconomicActivityType]:
    """ВЭД из справочника /refdata/ved (id > 0, текущая версия БД, display_order)."""
    q = economic_activity_type_query(sort_by="display_order", sort_dir="asc")
    if version_id is not None:
...
def _load_fd_ei_year_values_map(
    *,
    ...
    q = FederalDistrictElectricalIntensityYearParameter.query.filter(
    ...
    result: dict[tuple[int, str, int], Decimal | None] = {}
    for row in q.all():
        if row.year_number is None or not row.row_kind:
            continue
        result[
            (int(row.id_economic_activity_type), str(row.row_kind), int(row.year_number))
        ] = row.parameter_value
    return result
...
def _load_fd_ei_coef_map(
    *,
    ...
    q = FederalDistrictElectricalIntensityCoefficient.query.filter(
    ...
    for row in q.all():
        result[int(row.id_economic_activity_type)] = (
            row.coefficient_a,
            row.coefficient_x,
        )
    return result
...
def _build_fd_territory_summary(
    *,
    ...
    ved_consumption_cells = _sum_values_by_ved_year_for_veds(
        sum_ved_ids, consumption_by_ved_year, display_years
    )
    ...
    network_losses_cells = (
        {
            year: consumption_by_ved_year.get((int(network_losses_ved.id), year))
            for year in display_years
        }
    ...
    power_station_cells = (
        {
            year: consumption_by_ved_year.get((int(power_station_ved.id), year))
            for year in display_years
        }
    ...
    _enrich_ei_computed_rows(
        ...
        consumption_by_ved_year={
            (synthetic_ved_id, year): consumption_raw_cells.get(year)
            for year in display_years
        },
        product_output_by_ved_year={
            (synthetic_ved_id, year): vrp_cells.get(year) for year in display_years
        },
        accum_by_ved_year={
...
def _ved_label_for_log(
    ...
    *,
    ...
    if ved_by_id is not None:
        ...
        if ved is not None:
            return (ved.name or f"id={ved_id}").strip()
...
def save_electrical_intensity_from_post(form_data: Any) -> tuple[int, int]:
    ...
    for i in range(n):
        ...
        if rk in (ROW_KIND_INTENSITY, ROW_KIND_CALCULATED, ROW_KIND_DELTA):
