# -*- coding: utf-8 -*-
"""Накопленный выпуск продукции по ВЭД для РФ в целом (долгосрочный прогноз)."""
...
class RussiaFederationProductOutputParameter(db.Model, AuditMixin):
    __tablename__ = "gs_ekp_russia_federation_product_output_params"
    __table_args__ = (
    ...
    year = db.relationship(
        ...
        back_populates="rf_economic_activity_product_output_parameters",
        lazy="noload",
    )

    def __repr__(self) -> str:
        return (
            f"<RussiaFederationProductOutputParameter id={self.id} "
            f"ved={self.id_economic_activity_type} year={self.year_number}>"
        )
