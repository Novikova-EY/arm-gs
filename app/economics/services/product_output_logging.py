# -*- coding: utf-8 -*-
"""Журнал изменений таблицы выпуска продукции (модель Log)."""
...
ENTITY_TYPE = "long_term_product_output"
ACTION_TITLE = "выпуск продукции (долгосрочный прогноз)"
...
def load_product_output_logs_raw(
...
def load_formatted_product_output_logs(
    database_version_id: Optional[int], *, limit: int = 50
) -> list[dict]:
    logs = load_product_output_logs_raw(
        database_version_id, limit=limit, offset=0
    )
    return format_logs_for_display(logs)


def count_product_output_logs(database_version_id: Optional[int]) -> int:
    q = db.session.query(Log).filter(Log.entity_type == ENTITY_TYPE)
    if database_version_id is not None:
        q = q.filter(Log.entity_id == int(database_version_id))
    return int(q.count())
...
def log_product_output_cell_change(
    user: Any,
    *,
...
def log_product_output_excel_import(
    user: Any,
    stats: dict[str, Any],
    *,
    database_version_id: Optional[int],
