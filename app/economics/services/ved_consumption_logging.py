# -*- coding: utf-8 -*-
"""Журнал изменений таблицы потребления по ВЭД (модель Log)."""
...
ENTITY_TYPE = "long_term_ved_consumption"
ACTION_TITLE = "потребление по ВЭД (долгосрочный прогноз)"
...
def load_ved_consumption_logs_raw(
...
def load_formatted_ved_consumption_logs(
    database_version_id: Optional[int], *, limit: int = 50
) -> list[dict]:
    logs = load_ved_consumption_logs_raw(
        database_version_id, limit=limit, offset=0
    )
    return format_logs_for_display(logs)


def count_ved_consumption_logs(database_version_id: Optional[int]) -> int:
    q = db.session.query(Log).filter(Log.entity_type == ENTITY_TYPE)
    if database_version_id is not None:
        q = q.filter(Log.entity_id == int(database_version_id))
    return int(q.count())
...
def log_ved_consumption_cell_change(
    user: Any,
    *,
...
def log_ved_consumption_excel_import(
    user: Any,
    stats: dict[str, Any],
    *,
    database_version_id: Optional[int],
