# -*- coding: utf-8 -*-
"""Сервисы таблицы накопленных денежных доходов населения."""
...
from app.economics.models.federal_district_accum_monetary_income_parameter_model import (
    FederalDistrictAccumMonetaryIncomeParameter,
)
from app.extensions import db
from app.economics.services.accum_monetary_income_constants import (
    AMI_FD_EXCLUDED_NAME_KEYS,
    AMI_FD_PLACEHOLDER_NAME_KEYS,
)
from app.economics.services.accum_monetary_income_logging import (
    _fmt_log_value,
    log_accum_monetary_income_cell_change,
)
...
def _is_federal_district_excluded(fd: FederalDistrict) -> bool:
    for attr in ("name", "name_abr", "name_full"):
        ...
        if not key:
            continue
        if key in AMI_FD_EXCLUDED_NAME_KEYS or key in AMI_FD_PLACEHOLDER_NAME_KEYS:
            return True
    return False


def federal_districts_for_accum_monetary_income_page() -> list[FederalDistrict]:
    return [
        fd for fd in get_federal_district_list() if not _is_federal_district_excluded(fd)
    ]
...
def build_accum_monetary_income_page_context(
    *,
    ...
    return {
        "page_title": "Накопленные денежные доходы населения",
        "years": display_years,
...
def save_accum_monetary_income_from_post(form_data: Any) -> tuple[int, int]:
    version_id = get_current_version()
    user = _username()
    updated = 0
    skipped = 0
    ...
    for i in range(n):
        ...
        row.accum_monetary_income_mln_rub = dec
        ...
        log_accum_monetary_income_cell_change(
            ...
            detail_chunks=[
                ...
                f"год={year_n}",
                (
                    f"Накопленные денежные доходы населения, млн руб.: "
                    f"{_fmt_log_value(old_val)} → {_fmt_log_value(dec)}"
                ),
            ],
...
def _get_or_create_fd_row(
    ...
    filters = [
        FederalDistrictAccumMonetaryIncomeParameter.id_federal_district == fd_id,
        FederalDistrictAccumMonetaryIncomeParameter.year_number == year_n,
    ]
    if version_id is not None:
        filters.append(
            FederalDistrictAccumMonetaryIncomeParameter.database_version_id == version_id
        )
    row = FederalDistrictAccumMonetaryIncomeParameter.query.filter(and_(*filters)).first()
    if row is None:
        row = FederalDistrictAccumMonetaryIncomeParameter(
