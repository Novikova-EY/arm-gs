# -*- coding: utf-8 -*-
"""Импорт выпуска продукции из Excel."""
...
from app.economics.services.product_output_services import (
    _federal_districts_for_po_page,
...
def import_product_output_from_xlsx_bytes(raw: bytes) -> dict[str, Any]:
        ...
        for entry in parsed_rows:
            ...
            for year_n, val in entry["cells"]:
                ...
                if is_same_decimal(to_decimal(old_val), to_decimal(val)):
                    stats["cells_skipped"] += 1
                    continue
                row.accumulated_product_output_mln_rub = val
