# -*- coding: utf-8 -*-
"""Импорт накопленных инвестиций в основной капитал из Excel."""
...
from app.economics.services.accum_fixed_capital_services import (
    _federal_districts_for_afci_page,
...
def _build_ved_lookup(version_id: int | None) -> dict[str, int]:
    ...
    for ved in _ved_types_for_version(version_id):
        if ved.name:
            ...
            lookup[key] = ved.id
            if key == _normalize_label(TOTAL_ACCUM_FIXED_CAPITAL_NAME):
                total_ved_id = ved.id
...
def import_accum_fixed_capital_from_xlsx_bytes(raw: bytes) -> dict[str, Any]:
        ...
        for entry in parsed_rows:
            ...
            for year_n, val in entry["cells"]:
                ...
                if is_same_decimal(to_decimal(old_val), to_decimal(val)):
                    stats["cells_skipped"] += 1
                    continue
                row.accumulated_fixed_capital_investment_mln_rub = val
