# -*- coding: utf-8 -*-
"""Журнал изменений численности населения (модель Log)."""
...
ENTITY_TYPE = "long_term_population"
ACTION_TITLE = "численность населения (долгосрочный прогноз)"
...
def load_formatted_population_logs(
    database_version_id: Optional[int], *, limit: int = 50
) -> list[dict]:
    logs = load_population_logs_raw(database_version_id, limit=limit, offset=0)
    return format_logs_for_display(logs)


def count_population_logs(database_version_id: Optional[int]) -> int:
    q = db.session.query(Log).filter(Log.entity_type == ENTITY_TYPE)
    if database_version_id is not None:
        q = q.filter(Log.entity_id == int(database_version_id))
    return int(q.count())
...
def log_population_cell_change(
    user: Any,
    *,
...
def log_population_excel_import(
    user: Any,
    stats: dict[str, Any],
    *,
    database_version_id: Optional[int],
