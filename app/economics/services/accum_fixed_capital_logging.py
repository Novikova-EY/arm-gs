# -*- coding: utf-8 -*-
"""Журнал изменений таблицы накопленных инвестиций в основной капитал (модель Log)."""
...
ENTITY_TYPE = "long_term_accum_fixed_capital"
ACTION_TITLE = "накопленные Накопленные инвестиции в основной капитал (долгосрочный прогноз)"
...
def load_accum_fixed_capital_logs_raw(
...
def load_formatted_accum_fixed_capital_logs(
    database_version_id: Optional[int], *, limit: int = 50
) -> list[dict]:
    logs = load_accum_fixed_capital_logs_raw(
        database_version_id, limit=limit, offset=0
    )
    return format_logs_for_display(logs)


def count_accum_fixed_capital_logs(database_version_id: Optional[int]) -> int:
    q = db.session.query(Log).filter(Log.entity_type == ENTITY_TYPE)
    if database_version_id is not None:
        q = q.filter(Log.entity_id == int(database_version_id))
    return int(q.count())
...
def log_accum_fixed_capital_cell_change(
    user: Any,
    *,
...
def log_accum_fixed_capital_excel_import(
    user: Any,
    stats: dict[str, Any],
    *,
    database_version_id: Optional[int],
    ...
    parts = [
        ...
        f"строк_ВЭД={stats.get('rows_processed')}",
