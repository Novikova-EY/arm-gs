...
from app.economics.models.formula_text.product_output_formula_text_model import (
    ProductOutputFormulaText,
)
from app.extensions import db
from app.economics.services.product_output_formula_registry import (
    ProductOutputFormulaDef,
    get_formula_def,
    iter_formula_defs,
)

_OVERRIDE_CACHE_KEY = "lt_po_formula_text_overrides"


def _load_overrides_from_db() -> dict[str, str]:
    rows = ProductOutputFormulaText.query.all()
...
def po_formula_text(formula_key: str, default: str | None = None) -> str:
...
def save_formula_text_override(*, formula_key: str, formula_text: str) -> None:
    ...
    row = ProductOutputFormulaText.query.filter_by(formula_key=key).first()
    if row is None:
        row = ProductOutputFormulaText(formula_key=key, formula_text=text)
        db.session.add(row)
    else:
