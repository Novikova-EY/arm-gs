# -*- coding: utf-8 -*-
"""Импорт численности населения из Excel.

Поддерживаемые форматы:
- экспорт страницы (территория в столбце 1, годы с столбца 2);
- шаблон «Численность населения_для загрузки.xlsx» (лист «тысяч человек»:
  аббревиатура ФО в столбце 2, годы с столбца 3, шапка годов — строка 2).
"""
...
from app.economics.services.population_logging import log_population_excel_import
from app.economics.services.population_services import (
    _get_or_create_fd_row,
    _username,
    federal_districts_for_population_page,
)
...
# Подписи листа/шапки шаблона загрузки (без привязки к регистру).
_UPLOAD_SHEET_NAME_HINTS = ("тысяч человек", "тыс. чел", "численность")
_UPLOAD_HEADER_HINTS = ("тысяч", "численност", "населен")
...
def _build_fd_lookup() -> dict[str, int]:
    """Ключи: аббревиатура, краткое и полное имя ФО (нормализованные)."""
    lookup: dict[str, int] = {}
...
def _detect_territory_column(
    ...
    *,
    ...
    """Столбец с подписью ФО: 1 — экспорт UI, 2 — шаблон загрузки."""
    hits_col1 = 0
    hits_col2 = 0
...
def _parse_population_sheet(
    ...
    if not parsed_rows:
        raise ValueError(
            ...
            f"Проверьте лист «{sheet.title}»: ожидается аббревиатура ФО "
            f"в столбце {territory_col} и годы начиная со столбца {first_year_col}."
        )
...
def _ensure_years_in_versions(
    ...
    """Добавляет отсутствующие годы в gs_sys_years для каждой версии БД (FK year_number)."""
...
def import_population_from_xlsx_bytes(raw: bytes) -> dict[str, Any]:
    ...
    try:
        ...
        parsed_rows, parse_stats = _parse_population_sheet(sheet, fd_lookup)
    finally:
        wb.close()
        ...
        for entry in parsed_rows:
            ...
            for year_n, val in entry["cells"]:
                ...
                if is_same_decimal(to_decimal(old_val), to_decimal(val)):
                    stats["cells_skipped"] += 1
                    continue
                row.population_thousand_persons = val
