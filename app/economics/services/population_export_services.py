# -*- coding: utf-8 -*-
"""Экспорт таблицы численности населения в Excel."""
...
def build_population_excel_stream(context: dict[str, Any]) -> BytesIO:
    wb = Workbook()
    ws = wb.active
    ws.title = "Численность населения"
    ...
    ws.cell(row=1, column=1, value="Федеральный округ, тыс. чел.").font = (
        Font(bold=True)
    )
    ...
    row_idx = 2
    for pop_row in context.get("population_rows") or []:
        label = f"{pop_row.get('abbr', '')} — {pop_row.get('label', '')}"
        ws.cell(row=row_idx, column=1, value=label)
        cells = pop_row.get("cells") or {}
        for col_idx, year in enumerate(years, start=2):
            val = cells.get(year)
            if val is not None:
