...
def init_database_version_middleware(app):
    ...
    def load_current_version():
        ...
        if active_version:
            ...
            # Если активной версии нет, работаем со всеми данными (None)
            g.current_db_version = None
            if not request.endpoint or not request.endpoint.startswith('static'):
                current_app.logger.debug(f"[VERSION_MIDDLEWARE] Активная версия не найдена, установлено: None")
                print(f"[DEBUG] [VERSION_MIDDLEWARE] Активная версия не найдена, установлено: None")
