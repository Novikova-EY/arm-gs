...
# Точность вывода коэффициента k строк «Потери в сетях» и «С.н. электростанций» (блок «Всего», ФО).
EI_FD_TOTAL_K_DISPLAY_ROUNDING_DIGITS = 3
...
REF_ROW_FORMULA_KEY_BY_KIND: dict[str, str] = {
    REF_ROW_CONSUMPTION: "ei_ved_consumption_plan",
}

EI_VED_CONSUMPTION_PLAN_FORMULA_TOOLTIP = (
    "Потребление ЭЭ для годов «план» = Выпуск продукции × Электроёмкость (расчётная) / 1000, "
    "млн кВтч (выпуск — млн руб., электроёмкость — кВтч/тыс. руб.). "
...
EI_INDUSTRIAL_CONSUMPTION_FORMULA_TOOLTIP = (
    ...
    "(сумма строк «Потребление ЭЭ» по указанным ВЭД), млн кВтч. "
...
REF_ROW_FD_PLAN_FORMULA_KEY_BY_KIND: dict[str, str] = {
    REF_ROW_FD_NETWORK_LOSSES: "ei_fd_total_network_losses_plan",
    REF_ROW_FD_POWER_STATION: "ei_fd_total_power_station_plan",
}
...
EI_FD_TOTAL_NETWORK_LOSSES_FORMULA_TOOLTIP = (
    ...
    "для соответствующего федерального округа / 1000, млрд кВтч."
)

EI_FD_TOTAL_NETWORK_LOSSES_PLAN_FORMULA_TOOLTIP = (
    "Потери в сетях для годов «план» = Потребление ЭЭ ВЭД × k, млрд кВтч. "
...
EI_FD_TOTAL_POWER_STATION_PLAN_FORMULA_TOOLTIP = (
    "С.н. электростанций для годов «план» = Потребление ЭЭ ВЭД × k, млрд кВтч. "
    "Для остальных лет — значение строки «С.н. электростанций» со страницы «Потребление ЭЭ по ВЭД» "
    "для соответствующего федерального округа / 1000, млрд кВтч."
)

EI_FD_TOTAL_CONSUMPTION_FORMULA_TOOLTIP = (
    "Потребление ЭЭ = Потребление ЭЭ ВЭД + Потери в сетях + С.н. электростанций "
    "(сумма соответствующих строк блока «Всего»), млрд кВтч."
)
...
POP_REF_ROW_PLAN_FORMULA_KEY_BY_KIND: dict[str, str] = {
    REF_ROW_HOUSEHOLD_CONSUMPTION: "ei_pop_ref_household_consumption_plan",
}
...
EI_POP_REF_HOUSEHOLD_CONSUMPTION_PLAN_FORMULA_TOOLTIP = (
    "Потребление ЭЭ в домашних хозяйствах для годов «план» = "
    "Численность населения (на начало года) × Потребление ЭЭ на душу (расчётное)"
    "Для остальных лет — значение строки «Домашние хозяйства» "
    "со страницы «Потребление ЭЭ по ВЭД» для соответствующего федерального округа."
)
...
EI_POP_GRAPH_POINT_ROW_LABEL = (
    "Коэффициент эластичности потребления ЭЭ на душу населения "
    "по накопленным денежным доходам"
)

POP_ROW_LABEL_BY_KIND: dict[str, str] = {
    ROW_KIND_INTENSITY: "Потребление ЭЭ на душу населения",
    ROW_KIND_GRAPH_POINT: EI_POP_GRAPH_POINT_ROW_LABEL,
...
EI_POP_PER_CAPITA_FORMULA_TOOLTIP = (
    "Потребление ЭЭ на душу населения, кВтч/тыс. руб., по каждому году "
...
EI_POP_CALCULATED_FORMULA_TOOLTIP = (
    "Потребление ЭЭ на душу (расчётное), кВтч/тыс. руб., по каждому году "
    "(не позже года с признаком «текущий» включительно) = коэффициент A × "
    "(Накопленные денежные доходы населения)^коэффициент X "
    "Если коэффициент A не указан вручную, используется значение «Арасч»."
)
...
EI_POP_DELTA_FORMULA_TOOLTIP = (
    ...
    "Потребление ЭЭ на душу населения, кВтч/тыс. руб."
)

EI_POP_GRAPH_POINT_FORMULA_TOOLTIP = (
    f"{EI_POP_GRAPH_POINT_ROW_LABEL} по каждому году "
    "(не позже года с признаком «текущий» включительно) = "
    "LOG(Потребление ЭЭ на душу_Y / Потребление ЭЭ на душу_{Y−1}; "
...
EI_POP_COEFFICIENT_A_FORMULA_TOOLTIP = (
    ...
    "Если не указан, подставляется «Арасч»."
)
...
EI_POP_COEFFICIENT_A_COMPUTED_FORMULA_TOOLTIP = (
    ...
    "(N — год с признаком «текущий»), X — коэффициент X;"
    "Используется в расчётах, если коэффициент A не указан вручную."
)

EI_POP_COEFFICIENT_X_FORMULA_TOOLTIP = (
    f"Коэффициент X = среднее арифметическое значений строки «{EI_POP_GRAPH_POINT_ROW_LABEL}» "
...
EI_RF_TOTAL_CONSUMPTION_FORMULA_TOOLTIP = (
    ...
    "с.н. эл.станций (сумма соответствующих строк блока РФ), млрд кВтч."
)
...
EI_RF_POP_PER_CAPITA_FORMULA_TOOLTIP = (
    "Потребление ЭЭ на душу населения, тыс.кВтч/чел. = Потребление ЭЭ в домашних "
    "хозяйствах / Численность населения (на начало года) "
    "(потребление — млн кВтч, численность — тыс. чел.)."
)

EI_RF_GAES_FORMULA_TOOLTIP = (
    "ГАЭС = значение строки «Потребление электрической энергии ГАЭС на заряд, млн кВтч "
    "(всего)» со страницы «ГАЭС на заряд» / 1000, млрд кВтч."
)

EI_RF_CONSUMPTION_WITHOUT_GAES_FORMULA_TOOLTIP = (
    "Потребление без ГАЭС = Потребление (полезное) суммарно по ВЭД − ГАЭС, млрд кВтч."
)

EI_RF_NETWORK_LOSSES_FORMULA_TOOLTIP = (
    "потери в сетях = значение строки «Потери в сетях» для РФ со страницы "
    "«Потребление ЭЭ по ВЭД» / 1000, млрд кВтч."
)
...
EI_RF_GDP_INTENSITY_FORMULA_TOOLTIP = (
    "Электроемкость ВВП, кВтч/тыс. руб. = Потребление ЭЭ / ВВП "
    "(потребление — млрд кВтч, ВВП — млрд руб.; пересчёт в кВтч/тыс. руб. "
...
REF_ROW_RF_VED_FORMULA_KEY_BY_KIND: dict[str, str] = {
    ...
    REF_ROW_ACCUM_FIXED_CAPITAL: "ei_rf_ved_accum_fixed_capital",
}
...
EI_RF_VED_INTENSITY_FORMULA_TOOLTIP = (
    "Электроемкость ВЭД, кВтч/тыс. руб. = сумма строк «Электроемкость» "
    "по данному ВЭД по всем федеральным округам."
)

EI_RF_VED_ACCUM_FIXED_CAPITAL_FORMULA_TOOLTIP = (
    "Накопленные инвестиции в основной капитал = сумма строк "
    "«Накопленные инвестиции в основной капитал» по данному ВЭД "
