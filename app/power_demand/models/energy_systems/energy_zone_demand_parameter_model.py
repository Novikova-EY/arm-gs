# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для энергозоны (EnergyZone)."""
...
class EnergyZoneDemandParameter(db.Model, AuditMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_gs_energy_zone_demand_params_year_vs_hist",
        ),
    ...
    id_energy_zone = db.Column(
        db.Integer,
        db.ForeignKey(f"{SCHEMA_REFDATA}.gs_sys_energy_zones.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    energy_zone = db.relationship("EnergyZone", back_populates="demand_parameters")
