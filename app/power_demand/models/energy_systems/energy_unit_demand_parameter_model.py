# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для энергоузла (EnergyUnit)."""
...
class EnergyUnitDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    __tablename__ = "gs_pd_energy_unit_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_energy_unit_demand_params_year_vs_hist",
        ),
    ...
    energy_unit = db.relationship("EnergyUnit", back_populates="demand_parameters")
    ...
    def __repr__(self) -> str:
        return (
            f"<EnergyUnitDemandParameter id={self.id} energy_unit_id={self.id_energy_unit} "
