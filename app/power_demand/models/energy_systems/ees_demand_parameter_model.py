# -*- coding: utf-8 -*-
"""
Параметры нагрузки для ЭЭС (отдельный сценарий, без привязки к справочнику типов энергосистем).

Таблица уровня страны/агрегата — как gs_russia_federation_demand_params.
"""
...
class EesDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    __tablename__ = "gs_pd_ees_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_ees_demand_params_year_vs_hist",
        ),
    ...
    def __repr__(self) -> str:
        return (
            f"<EesDemandParameter id={self.id} "
