# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для объединённой энергосистемы (UnionEnergySystem)."""
...
class UnionEnergySystemDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="ck_gs_union_energy_system_demand_params_year_vs_hist",
        ),
    ...
    union_energy_system = db.relationship(
        "UnionEnergySystem", back_populates="demand_parameters"
    )
    ...
    # Коэффициент k для показателя «Расчетный совмещенный максимум потребления мощности ОЭС на ЕЭС, МВт»
