# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для типа энергосистемы (EnergySystemType)."""
...
class EnergySystemTypeDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    __tablename__ = "gs_pd_energy_system_type_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_energy_system_type_demand_params_year_vs_hist",
        ),
    ...
    energy_system_type = db.relationship(
        "EnergySystemType", back_populates="demand_parameters"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<EnergySystemTypeDemandParameter id={self.id} "
