# -*- coding: utf-8 -*-
"""Параметры нагрузки для Централизованной зоны (без FK на справочник)."""
...
class CentralizedZoneDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    __tablename__ = "gs_pd_centralized_zone_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_centralized_zone_demand_params_year_vs_hist",
        ),
    ...
    def __repr__(self) -> str:
        return (
            f"<CentralizedZoneDemandParameter id={self.id} "
