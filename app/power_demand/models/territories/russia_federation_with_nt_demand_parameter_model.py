# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для РФ с НТ (агрегат)."""
...
class RussiaFederationWithNtDemandParameter(db.Model, AuditMixin):
    __tablename__ = "gs_pd_russia_federation_with_nt_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_russia_federation_with_nt_demand_params_year_vs_hist",
        ),
    ...
    def __repr__(self) -> str:
        return (
            f"<RussiaFederationWithNtDemandParameter id={self.id} "
