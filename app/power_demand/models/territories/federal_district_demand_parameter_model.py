# -*- coding: utf-8 -*-
"""Параметры нагрузки (demand) для федерального округа (FederalDistrict)."""
...
class FederalDistrictDemandParameter(db.Model, AuditMixin, PerimeterVariantColumnMixin):
    __tablename__ = "gs_pd_federal_district_demand_params"
    __table_args__ = (
        CheckConstraint(
            "(is_historical_maximum = true AND year_number IS NULL) OR "
            "(is_historical_maximum = false AND year_number IS NOT NULL)",
            name="ck_gs_federal_district_demand_params_year_vs_hist",
        ),
    ...
    federal_district = db.relationship(
        "FederalDistrict", back_populates="demand_parameters"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<FederalDistrictDemandParameter id={self.id} "
