...
def aggregate_power_by_regional_district(pagination, rounding_digits, start_year, end_year):
    ...
    for regional_district_id, years_data in p_ust_source.items():
        for year, value in years_data.items():
            if start_year <= year <= end_year:
                p_ust[regional_district_id][year] = value or Decimal(0)

    for regional_district_id, years_data in p_ogr_source.items():
        for year, value in years_data.items():
            if start_year <= year <= end_year:
                p_ogr[regional_district_id][year] = value or Decimal(0)

    for regional_district_id, years_data in p_rasp_source.items():
        for year, value in years_data.items():
            if start_year <= year <= end_year:
                p_rasp[regional_district_id][year] = value or Decimal(0)
...
def aggregate_regional_districts_by_station_types(pagination, rounding_digits, start_year, end_year):
    ...
    for station in stations:
        ...
        if regional_district_id is None:
            continue
        ...
        for machine in station.machines:
            ...
            for year_raw, powers in powers_by_year.items():
                ...
                p_ust[regional_district_id][station_type_id][year] += Decimal(powers.get("p_ust") or 0)
                p_ogr[regional_district_id][station_type_id][year] += Decimal(powers.get("p_ogr") or 0)
                p_rasp[regional_district_id][station_type_id][year] += Decimal(powers.get("p_rasp") or 0)
...
def aggregate_regional_districts_by_station_types_with_fuel(pagination, rounding_digits, start_year, end_year):
    stations = pagination["stations"]
    ...
    for station in stations:
        ...
        for machine in station.machines:
            ...
            for year_raw, powers in powers_by_year.items():
                ...
                # ==== Агрегация ====
                p_ust[regional_district_id][fuel_type_id][year] += Decimal(powers.get("p_ust") or 0)
                p_ogr[regional_district_id][fuel_type_id][year] += Decimal(powers.get("p_ogr") or 0)
                p_rasp[regional_district_id][fuel_type_id][year] += Decimal(powers.get("p_rasp") or 0)
...
def aggregate_regional_districts_by_tes_types(pagination, rounding_digits, start_year, end_year):
    stations = pagination["stations"]
            ...
            for tes_type_link in machine.machine_tes_types:
                ...
                for year, powers in (machine.powers_by_year or {}).items():
                    if start_year <= year <= end_year and tes_type_link.year_number == year:
                        p_ust[regional_district_id][tes_type_id][year] += Decimal(powers.get("p_ust", 0) or 0)
                        p_ogr[regional_district_id][tes_type_id][year] += Decimal(powers.get("p_ogr", 0) or 0)
                        p_rasp[regional_district_id][tes_type_id][year] += Decimal(powers.get("p_rasp", 0) or 0)
...
def aggregate_regional_districts_by_tes_types_with_fuel(pagination, rounding_digits, start_year, end_year):
    stations = pagination["stations"]
    ...
    for station in stations:
        ...
        for machine in station.machines:
            ...
            for year_raw, powers in powers_by_year.items():
                ...
                # ==== Агрегация ====
                p_ust[regional_district_id][tes_type_id][fuel_type_id][year] += Decimal(powers.get("p_ust") or 0)
                p_ogr[regional_district_id][tes_type_id][fuel_type_id][year] += Decimal(powers.get("p_ogr") or 0)
                p_rasp[regional_district_id][tes_type_id][fuel_type_id][year] += Decimal(powers.get("p_rasp") or 0)
...
def aggregate_regional_districts_by_tes_machine_types(pagination, rounding_digits, start_year, end_year):
    stations = pagination["stations"]
    ...
    for station in stations:
        ...
        for machine in station.machines:
            ...
            for year_raw, powers in powers_by_year.items():
                ...
                # ==== Агрегация ====
                p_ust[regional_district_id][tes_type_id][machine_type_id][year] += Decimal(powers.get("p_ust") or 0)
                p_ogr[regional_district_id][tes_type_id][machine_type_id][year] += Decimal(powers.get("p_ogr") or 0)
                p_rasp[regional_district_id][tes_type_id][machine_type_id][year] += Decimal(powers.get("p_rasp") or 0)
...
def aggregate_regional_districts_by_tes_machine_types_with_fuel(pagination, rounding_digits, start_year, end_year):
    stations = pagination["stations"]
    ...
    for station in stations:
        ...
        for machine in station.machines:
            ...
            for year_raw, powers in powers_by_year.items():
                ...
                # ==== Агрегация ====
                p_ust[regional_district_id][tes_type_id][tes_machine_type_id][fuel_type_id][year] += Decimal(powers.get("p_ust") or 0)
                p_ogr[regional_district_id][tes_type_id][tes_machine_type_id][fuel_type_id][year] += Decimal(powers.get("p_ogr") or 0)
                p_rasp[regional_district_id][tes_type_id][tes_machine_type_id][fuel_type_id][year] += Decimal(powers.get("p_rasp") or 0)
