...
def aggregate_power_by_energy_system_types(rows):
    ...
    for row in rows:
        ...
        p_ogr[est][year] += row.p_ogr or Decimal(0)
        p_rasp[est][year] += row.p_rasp or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_station_types(rows):
    ...
    for row in rows:
        ...
        p_ogr[est][station_type][year] += row.p_ogr or Decimal(0)
        p_rasp[est][station_type][year] += row.p_rasp or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_station_types_with_fuel(rows):
    ...
    for row in rows:
        ...
        p_ogr[est][station_type][fuel][year] += row.p_ogr or Decimal(0)
        p_rasp[est][station_type][fuel][year] += row.p_ogr or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_tes_types(rows):
    p_ust = defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal)))
    p_ogr = defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal)))
    p_rasp = defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal)))
    ...
    for row in rows:
        ...
        p_ogr[est][tes_type][year] += row.p_ogr or Decimal(0)
        p_rasp[est][tes_type][year] += row.p_ogr or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_tes_types_with_fuel(rows):
    p_ust = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    p_ogr = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    p_rasp = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    ...
    for row in rows:
        ...
        p_ogr[est][tes_type][fuel][year] += row.p_ogr or Decimal(0)
        p_rasp[est][tes_type][fuel][year] += row.p_rasp or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_tes_machine_types(rows):
    p_ust = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    p_ogr = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    p_rasp = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(Decimal))))
    ...
    for row in rows:
        ...
        p_ogr[est][tes_type][tes_machine_type][year] += row.p_ogr or Decimal(0)
        p_rasp[est][tes_type][tes_machine_type][year] += row.p_rasp or Decimal(0)

    return {"aggregated": {"p_ust": p_ust, "p_ogr": p_ogr, "p_rasp": p_rasp}}


def aggregate_energy_system_types_by_tes_machine_types_with_fuel(rows):
    ...
    for row in rows:
        ...
        p_ogr[est][tes_type][tes_machine_type][fuel_type][year] += row.p_ogr or Decimal(0)
        p_rasp[est][tes_type][tes_machine_type][fuel_type][year] += row.p_rasp or Decimal(0)
