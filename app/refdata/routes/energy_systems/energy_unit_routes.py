...
def energy_unit_list():
    ...
    if request.method == "POST":
        ...
        energy_unit_names_dat       = request.form.getlist("energy_unit_names_dat[]")
        ...
        if len(energy_unit_names_dat) < n:
            energy_unit_names_dat = list(energy_unit_names_dat) + [""] * (n - len(energy_unit_names_dat))
        ...
        if energy_unit_delete:
            try:
                ...
                deleted_ids = {int(item) for item in energy_unit_delete if item}
        ...
        try:
           ...
           # Формирование данных для обновления
            energy_unit_data = []
            for energy_unit_id, energy_unit_name, name_rp, name_dp, name_dat, regional_district_id, regional_energy_system_id in zip(
                energy_unit_ids, energy_unit_names, energy_unit_names_rp, energy_unit_names_dp, energy_unit_names_dat, regional_district_ids, regional_energy_system_ids
            ):
                ...
                try:
                    energy_unit_data.append({
                        ...
                        "name_dat": (name_dat or "").strip() if name_dat is not None else "",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            payload = [{
                    ...
                    "name_dat": (form.name_dat.data or "").strip(),
