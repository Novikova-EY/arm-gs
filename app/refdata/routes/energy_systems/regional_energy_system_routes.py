...
# Формы
from app.refdata.forms.energy_systems.regional_energy_system_forms import RegionalEnergySystemFilterForm, AddRegionalEnergySystemForm
...
from app.refdata.services.energy_systems.regional_energy_system_services import (
    ...
    import_regional_energy_system_from_excel_service, 
    export_regional_energy_system_to_excel_service, 
)
...
def add_regional_energy_system_routes():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            new_regional_energy_system_id = add_regional_energy_system_service([
                {
                    "name": form.name.data,
...
@refdata_bp.route("/import_regional_energy_system_to_sql", methods=["POST"])
def import_regional_energy_system_to_sql_routes():
    ...
    try:
        imported_count = import_regional_energy_system_from_excel_service(file, user)
...
@refdata_bp.route("/export_regional_energy_system_to_excel", methods=["GET"])
def export_regional_energy_system_to_excel_routes():
    ...
    try:
        ...
        excel_data = export_regional_energy_system_to_excel_service(user, regional_energy_system_filter, union_energy_system_filter, sort_by, sort_dir)
