"""Формы для справочника «Типы состояния (ConditionType)».
"""
...
class ConditionTypeFilterForm(FlaskForm):
    ...
    name = StringField(
        "Тип состояния",
        validators=[
            DataRequired(message="Поле «Тип состояния» обязательно."),
    ...
    condition_type_filter = StringField(
        ...
        render_kw={"placeholder": "Поиск по типам состояния"},
    )
...
class AddConditionTypeForm(FlaskForm):
    csrf_token = HiddenField()
