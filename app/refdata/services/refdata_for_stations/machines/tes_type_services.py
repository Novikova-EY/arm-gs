...
def tes_type_query(
    ...
    query = apply_version_filter(query, TesType)
    ...
    if sort_by == "name":
        ...
        if sort_dir == "desc":
            query = query.order_by(
                (TesType.display_order.is_(None)),
                TesType.display_order.desc(),
            )
        else:
            query = query.order_by(
                (TesType.display_order.is_(None)),
                TesType.display_order.asc(),
            )
    else:
...
def update_tes_type_service(data, user):
    ...
    with db.session.no_autoflush:
        for record in data:
            ...
            if name != (obj.name or ""):
                q = (apply_version_filter(TesType.query, TesType)
            ...
            if display_order is not None and display_order != obj.display_order:
                q_display = (
                    apply_version_filter(TesType.query, TesType)
                    .filter(
                        TesType.display_order == display_order,
                        TesType.id != tes_type_id,
                    )
                )
                if q_display.first():
    ...
    def _do_insert():
        with db.session.no_autoflush:
            for record in data:
                ...
                dup = (apply_version_filter(TesType.query, TesType)
                       .filter(TesType.name == name)
                ...
                if display_order is not None:
                    dup_display = (
                        apply_version_filter(TesType.query, TesType)
                        .filter(TesType.display_order == display_order)
                        .with_for_update()
                        .first()
                    )
                    if dup_display:
                ...
                obj = TesType(name=name, display_order=display_order)
...
# === all_versions_tes_type start ===
@no_autoflush
def add_tes_type_all_versions_service(data, user):
...
@no_autoflush
def update_tes_type_all_versions_service(data, user):
    from app.refdata.services.refdata_all_versions_common import add_all_versions_records, update_all_versions_records, fk_id_for_version
