...
def tes_machine_type_query(
    ...
    query = apply_version_filter(query, TesMachineType)
...
def update_tes_machine_type_service(data, user):
    ...
    with db.session.no_autoflush:
        for record in data:
            ...
            if name != (obj.name or ""):
                q = (apply_version_filter(TesMachineType.query, TesMachineType)
    ...
    def _do_insert():
        with db.session.no_autoflush:
            ...
            for record in data:
                ...
                dup = (apply_version_filter(TesMachineType.query, TesMachineType)
                       .filter(TesMachineType.name == name)
