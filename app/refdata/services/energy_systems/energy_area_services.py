...
def energy_area_query(
    ...
    query = apply_version_filter(query, EnergyArea)
...
def update_energy_area_service(data, user):
    ...
    log_to_db(
        user, 
        "Получены данные для обновления списка энергорайонов", 
        f"{data}",
           ...
           # Проверки на валидность данных
            ...
            if not obj:
                log_to_db(
                    user, 
                    "Ошибка валидации", 
                    f"Запись с ID «{energy_area_id}» не найдена.", 
                    entity_type="energy_area", 
                    entity_id=energy_area_id)
            ...
            if name != (obj.name or ""):
                q = (apply_version_filter(EnergyArea.query, EnergyArea)
    ...
    try:
        ...
        if updated_ids:
            log_to_db(
                user, 
                "Сохранены изменения по энергорайонам", 
                f"Измененных записей: {len(updated_ids)} (id: {updated_ids})", 
                entity_type="energy_area")
        else:
            log_to_db(
                user, 
                "Изменений по энергорайонам не обнаружено", 
                "", 
                entity_type="energy_area")
        ...
        log_to_db(
            user, 
            "Ошибка сохранения энергорайона(уникальность/целостность)", 
            str(e), 
            entity_type="energy_area")
        ...
        log_to_db(
            user, 
            "Неизвестная ошибка при сохранении энергорайонов", 
            str(e), 
            entity_type="energy_area")
...
def add_energy_area_service(data, user):
    ...
    def _do_insert():
        with db.session.no_autoflush:
            ...
            for record in data:
                ...
                dup = (apply_version_filter(EnergyArea.query, EnergyArea)
                        .filter(EnergyArea.name == name)
    ...
    try:
        ...
        db.session.rollback()
        quick_fix_seq(SCHEMA_REFDATA, "energy_areas")
        _do_insert()
        _commit_with_retry()
        return None
    except Exception as e:
        ...
        log_to_db(
            user, 
            "Ошибка сохранения энергорайонв", 
            str(e), 
            entity_type="energy_area")
...
def export_energy_area_service(
    ...
    log_to_db(
        user, 
        "Начата выгрузка таблицы энергорайонов. Параметры экспорта",
        (
    ...
    log_to_db(
        user, 
        "Подготовка данных для экспорта таблицы энергорайонов в Excel",
    ...
    log_to_db(
        user, 
        "Экспорт таблицы энергорайонов в Excel завершен", 
